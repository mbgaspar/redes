#include <ESP8266WiFi.h>
#include <WiFiClientSecure.h>
#include <MQTT.h>
#include <time.h>
#include <string.h>

//enable only one of these below, disabling both is fine too.
#define CHECK_CA_ROOT
//#define CHECK_PUB_KEY
//#define CHECK_FINGERPRINT
////--------------------------////

#define rele 4 //Definição do pino correspondente ao rele
#ifndef SECRET

// Login do wifi que a esp deve se conectar
const char ssid[] = "redes_mqtt";
const char pass[] = "redes22108";

// Nome da ESP para po broker
#define HOSTNAME "esp8266_b103"

// IP do broker
const char MQTT_HOST[] = "172.16.158.155";
// Porta para se usar com certificado
const int MQTT_PORT = 8883;
const char MQTT_USER[] = ""; // leave blank if no credentials used
const char MQTT_PASS[] = ""; // leave blank if no credentials used

//Definição do topico que sera feito o subscribe e publish
const char MQTT_SUB_TOPIC[] = "blocob/b103";
const char MQTT_PUB_TOPIC[] = "blocob/b103";

// String pré inicializada que envia o estado atual do rele
String msg = "LO";

#ifdef CHECK_CA_ROOT
static const char digicert[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
    )EOF";
#endif

#ifdef CHECK_PUB_KEY
// Extracted by: openssl x509 -pubkey -noout -in ca.crt
static const char pubkey[] PROGMEM = R"KEY(
    -----BEGIN PUBLIC KEY-----
30 82 01 0a 02 82 01 01 00 c1 34 1c a9 88 cd f4 ce c2 42 8b 
4f 74 c7 1d ef 8e 6d d8 b3 6a 63 e0 51 99 83 eb 84 df df 32 
5d 35 e6 06 62 7e 02 11 76 f2 3f a7 f2 de d5 9c f1 2d 9b a1 
6e 9d ce b1 fc 49 d1 5f f6 ea 37 db 41 89 03 d0 7b 53 51 56 
4d ed f1 75 af cb 9b 72 45 7d a1 e3 91 6c 3b 8c 1c 1c 6a e4 
19 8e 91 88 34 76 a9 1d 19 69 88 26 6c aa e0 2d 84 e8 31 5b 
d4 a0 0e 06 25 1b 31 00 b3 4e a9 90 41 62 33 0f aa 0d f2 e8 
fe cc 45 28 1e af 42 51 5e 90 c7 82 ca 68 cb 09 b3 70 3c 9c 
aa ca 11 66 3d 6c 22 a3 f3 c3 32 bb 81 4f 33 c7 dd c8 a8 06 
7a c9 58 a5 dc dc e8 d7 74 b1 85 24 e7 e3 ee 93 f4 8f f7 6b 
d8 b1 fb d9 e4 af bf 73 d0 40 59 7d d0 26 4f 16 1a c2 51 c4 
47 49 2c 68 13 ac a3 18 e7 67 cf b7 fa 3e f7 8b 20 1e 7b e2 
44 0e 47 0b 7c 78 f9 f4 ca 27 6b 4c 2d 62 72 d8 a4 10 3d e7 
1d 88 4c 50 e5 02 03 01 00 01
    -----END PUBLIC KEY-----
    )KEY";
#endif

#ifdef CHECK_FINGERPRINT
// Extracted by: openssl x509 -fingerprint -in ca.crt
static const char fp[] PROGMEM = "06:90:7E:A4:Ec:A0:F7:70:28:12:DC:51:1F:ED:62:A7:99:AC:71:9A";
#endif
#endif
//////////////////////////////////////////////////////

#if (defined(CHECK_PUB_KEY) and defined(CHECK_CA_ROOT)) or (defined(CHECK_PUB_KEY) and defined(CHECK_FINGERPRINT)) or (defined(CHECK_FINGERPRINT) and defined(CHECK_CA_ROOT)) or (defined(CHECK_PUB_KEY) and defined(CHECK_CA_ROOT) and defined(CHECK_FINGERPRINT))
#error "cant have both CHECK_CA_ROOT and CHECK_PUB_KEY enabled"
#endif

//Definição do cliente seguro que trabalha com protocolo TLS
BearSSL::WiFiClientSecure net;
//Definição do client MQTT
MQTTClient client;

//Variaveis para se contar o tempo
unsigned long lastMillis = 0;
time_t now;

void mqtt_connect()
{
  Serial.print("checking wifi...");
  while (WiFi.status() != WL_CONNECTED)
  {
    Serial.print(".");
    delay(1000);
  }

  Serial.print("\nMQTT connecting ");
  while (!client.connect(HOSTNAME))
  {
    Serial.print(".");
    delay(1000);
  }

  Serial.println("connected!");

  client.subscribe(MQTT_SUB_TOPIC);
}

//Função que trata o recebimento de informações do broker
void messageReceived(String topic, String message)
{
  Serial.print("Message arrived on topic: ");
  Serial.print(topic);
  Serial.print(". Message: ");
  for (int i = 0; i < message.length(); i++) {
    Serial.print(message[i]);
  }
  String messageTemp;

  messageTemp = message;

  Serial.println();

  // If a message is received on the topic blocob/b103,
  // you check if the message is either "HI" or "LO".
  // Changes the output state according to the message
  if (String(topic) == "blocob/b103")
  {
    Serial.print("Changing output to ");

    if (messageTemp == "HI" )
    {
      Serial.println("off");
      digitalWrite(LED_BUILTIN, HIGH);
      digitalWrite(rele, HIGH);
      msg = messageTemp;
    }
    else if (messageTemp == "LO")
    {
      Serial.println("on");
      digitalWrite(LED_BUILTIN, LOW);
      digitalWrite(rele, LOW);
      msg = messageTemp;
    }
  }
}

void setup()
{
  Serial.begin(115200);
  Serial.println();
  Serial.println();
  pinMode(LED_BUILTIN, OUTPUT); //Define led mode
  pinMode(rele, OUTPUT); // Define o pino do rele e seu estado inicial
  digitalWrite(rele, LOW);
  Serial.print("Attempting to connect to SSID: ");
  Serial.print(ssid);
  WiFi.hostname(HOSTNAME);
  WiFi.mode(WIFI_STA);
  WiFi.begin(ssid, pass);
  while (WiFi.status() != WL_CONNECTED)
  {
    Serial.print(".");
    delay(1000);
  }
  Serial.println("connected!");
  Serial.println(WiFi.localIP());
  Serial.print("Setting time using SNTP");
  configTime(-5 * 3600, 0, "pool.ntp.org", "time.nist.gov");
  now = time(nullptr);
  while (now < 1510592825)
  {
    delay(500);
    Serial.print(".");
    now = time(nullptr);
  }
  Serial.println("done!");
  struct tm timeinfo;
  gmtime_r(&now, &timeinfo);
  Serial.print("Current time: ");
  Serial.print(asctime(&timeinfo));

#ifdef CHECK_CA_ROOT
  BearSSL::X509List cert(digicert);
  net.setTrustAnchors(&cert);
#endif
#ifdef CHECK_PUB_KEY
  BearSSL::PublicKey key(pubkey);
  net.setKnownKey(&key);
#endif
#ifdef CHECK_FINGERPRINT
  net.setFingerprint(fp);
#endif
#if (!defined(CHECK_PUB_KEY) and !defined(CHECK_CA_ROOT) and !defined(CHECK_FINGERPRINT))
  net.setInsecure();
#endif

  //Inicia o cliente MQTT
  client.begin(MQTT_HOST, MQTT_PORT, net);
  //Definição da função que ira tratar as informações recebidas
  client.onMessage(messageReceived);

  mqtt_connect();
}

void loop()
{
  now = time(nullptr);
  if (WiFi.status() != WL_CONNECTED)
  {
    Serial.print("Checking wifi");
    while (WiFi.waitForConnectResult() != WL_CONNECTED)
    {
      WiFi.begin(ssid, pass);
      Serial.print(".");
      delay(10);
    }
    Serial.println("connected");
  }
  else
  {
    if (!client.connected())
    {
      mqtt_connect();
    }
    else
    {
      client.loop();
    }
  }

  // publish a message roughly every 1 min.
  if (millis() - lastMillis > 60000)
  {
    lastMillis = millis();
    client.publish(MQTT_PUB_TOPIC, msg, false, 0);
    Serial.print("Mensagem publicada no tópico:");
    Serial.println(MQTT_PUB_TOPIC);
    Serial.print("Mensagem publicada:");
    Serial.println(msg);
  }
}
