...
bool rele = true; // Marca o estado inicial do relé "ligado"
...
void tarefa_rele() {
  ...
  if (Serial.available()) {
    ...
    if (dado_recebido == 'r') {// abre ou fecha o portão
      if (rele == true) {
        rele = false;
...
void tarefa_status() {
  if (millis() - lastStatus > 5000)
  {
    ...
    if (rele == true) {
      ...
      Serial.print("\n");
      client.publish(MQTT_PUB_TOPIC, ctime(&now), false, 0);
      client.publish(MQTT_PUB_TOPIC2, "relé ligado", false, 0);
    }
    else {
      ...
      Serial.print("\n");
      client.publish(MQTT_PUB_TOPIC, ctime(&now), false, 0);
      client.publish(MQTT_PUB_TOPIC2, "relé desligado", false, 0);
    }
  }
}
